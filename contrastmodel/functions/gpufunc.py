...
def normalized_conv(img, filt, padval):
    ...
    FFTPlan(shape=pad_img.shape, itype=np.float32, otype=np.complex64)
    ...
    normimg = np.ones(pad_filt.shape, dtype=np.float32)
    normtemp1 = np.zeros(pad_filt.shape, dtype=np.complex64)  # to store output of FFT
    normtemp2 = np.zeros(pad_filt.shape, dtype=np.complex64)  # to store output of FFT
    normtemp3 = np.zeros(pad_img.shape, dtype=np.complex64)  # to store output of FFT
    abs_pad_filt = np.absolute(pad_filt)
    #abs_pad_filt_temp = np.zeros(abs_pad_filt.shape, dtype=np.complex64)  # to store output of FFT
    #abs_pad_filt = abs_pad_filt.astype(np.complex64)
    ...
    #d_abs_pad_filt_temp = cuda.to_device(abs_pad_filt_temp)
    ...
    temp = d_normtemp2.copy_to_host().real  # for testing

    d_pad_filt = cuda.to_device(pad_filt)
    d_pad_img = cuda.to_device(pad_img)
    fft(d_pad_filt, d_normtemp1)
    fft(d_pad_img, d_normtemp3)
    temp1 = d_normtemp1.copy_to_host().real
    temp2 = d_normtemp2.copy_to_host().real
    temp3 = d_normtemp3.copy_to_host()
    temp4 = d_pad_img.copy_to_host().real
    vmult(d_normtemp3, d_normtemp1, out=d_normtemp3)
    ifft(d_normtemp3, d_normtemp1)
    # temp_out = (cuda.fft.ifft_inplace(cuda.fft.fft_inplace(pad_img)) * cuda.fft.fft_inplace(pad_filt)).real
    vdiv(d_normtemp1, d_normtemp2, out=d_normtemp1)

    temp_out = d_normtemp1.copy_to_host().real
